...
local function do_configure(self,device)
  ...
  if device.manufacturer ~= nil then return end
...
local function humidity_attr_handler(driver, device, value, zb_rx)
  ...
  device:emit_event_for_endpoint(zb_rx.address_header.src_endpoint.value, capabilities.relativeHumidityMeasurement.humidity(utils.round(value.value / 100.0))) -- + device.preferences.humidityOffset))
   ...
   if Child_devices_created[device.id .. "-Thermostat"] ~= nil then
    Child_devices_created[device.id .. "-Thermostat"]:emit_event(capabilities.relativeHumidityMeasurement.humidity(utils.round(value.value / 100.0) + device.preferences.humidityOffset))
  end
end
