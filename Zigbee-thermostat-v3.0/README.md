...
It is automatically activated when manual or away modes are selected, so the programmed automations will not change the settings set.
...
2. Added temperature presets for ECO Mode in preferences

3. Added in preferences an option to choose the scale of steps for the adjustment of the temperature set points. The stock presentation of +/- 1º is not very precise, especially for ºC. Have to save energy!!
...
## List of supported devices (dic 03)
