...
local function do_added(driver, device)
  ...
  if device.manufacturer ~= nil then -- Is a child device
    ...
    if component == "main" then
      ...
      if device.vendor_provided_label == "child-switch-level" then
        local child_level = Parent_devices[device.parent_device_id]:get_latest_state(component, capabilities.switchLevel.ID, capabilities.switchLevel.level.NAME)
...
local function default_response_handler(driver, device, zb_rx)
  ...
  local function switch_level_handler(self,device,command)
    ...
    if device.manufacturer == nil then    ---- device.manufacturer == nil is NO Child device
      ...
      if Child_devices_created[device.id .. component] ~= nil and component ~= "main" then
        --print("<<< Child_devices_created[device.id .. component].vendor_provided_label >>>", Child_devices_created[device.id .. component].vendor_provided_label)
        if Child_devices_created[device.id .. component].vendor_provided_label == "child-switch-level" then
          Child_devices_created[device.id .. component]:emit_event(capabilities.switchLevel.level(on_Level))
        end
      end
    else
      --print("device.parent_device_id >>>",device.parent_device_id)
      ...
      if component ~= "main" then
       Parent_devices[device.parent_device_id]:send_to_component(component, zcl_clusters.Level.commands.MoveToLevelWithOnOff(Parent_devices[device.parent_device_id], math.floor(on_Level/100.0 * 254), 0xFFFF))
      end
    end
...
local function level_attr_handler(driver, device, value, zb_rx)
  ...
  if device.manufacturer == nil then    ---- device.manufacturer == nil is NO Child device
    ...
    local component = device:get_component_id_for_endpoint(zb_rx.address_header.src_endpoint.value)
    if Child_devices_created[device.id .. component] ~= nil and component ~= "main" then
      --print("<<< device.vendor_provided_label >>>", device.vendor_provided_label)
      if Child_devices_created[device.id .. component].vendor_provided_label == "child-switch-level" then
        Child_devices_created[device.id .. component]:emit_event(capabilities.switchLevel.level(math.floor((value.value / 254.0 * 100) + 0.5)))
      end
    end
  else
