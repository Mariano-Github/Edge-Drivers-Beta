...
local can_handle = function(opts, driver, device)
  --print("device.parent_assigned_child_key:",device.parent_assigned_child_key)
  if device.network_type == "DEVICE_EDGE_CHILD" and device.parent_assigned_child_key ~= "config" then
...
local function do_init (self, device)
  ...
  if device:get_field("cooling_Setpoint") == nil then device:set_field("cooling_Setpoint", device.preferences.coolTempAuto, {persist = true}) end
