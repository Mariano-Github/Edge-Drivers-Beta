...
-- this driver has the infoChange for preferences redirection for all no child devices and preferences for config child device
-- "device.preferences.configChild ~= nil" allow NO child devices infoChange and Version, Configuration, Assiciation command class Handlers
local can_handle = function(opts, driver, device)
  --if device.network_type == "DEVICE_EDGE_CHILD" and device.preferences.changeConfigProfile ~= nil then
  if device.preferences.configChild ~= nil or device.preferences.changeConfigProfile ~= nil then
    --local subdriver = require("device-config")
...
local function debug_pretty_print(device, parent_device)
  ...
  if device.network_type == "DEVICE_EDGE_CHILD" then
    ...
    if parent_device:get_field("firmVersion") ~= nil then
      outputString = outputString .."<b>Firmware Ver: </b>" .. parent_device:get_field("firmVersion") .. "<BR>"
    end
    if parent_device:get_field("library") ~= nil then
      outputString = outputString .."<b>Library Ver: </b>" .. parent_device:get_field("library") .. "<BR>"
    end
    if parent_device:get_field("zwProtocol") ~= nil then
      outputString = outputString .."<b>Zw Protocol Ver: </b>" .. parent_device:get_field("zwProtocol") .. "<BR>"
    end
    if (parent_device.zwave_manufacturer_id ~= nil and parent_device.zwave_product_type ~= nil and parent_device.zwave_product_id ~= nil) then
      ...
      --print("<<device.zwave_product_type:",device.zwave_product_type)
      --print("<<device.zwave_product_id:",device.zwave_product_id)
    ...
    --[[
    --local store = parent_device.st_store.data
    --print("device.st_store.data: >>>>>>",utils.stringify_table(store))
...
local function added_handler(self, device)
  if device.network_type == "DEVICE_EDGE_CHILD" then
    ...
    elseif device.preferences.changeConfigProfile == "WakeUp" then
      --device:emit_event(wake_Up_Value.wakeUpValue(wake_up_value_set))
      device:emit_event(command_Config.commandConfig({value = device:get_field("config_action")}, {visibility = {displayed = false }, state_change = true}))
...
local function do_init(self, device)
  print("<<<< do_init in device-config subdriver >>>>")
  if device.network_type == "DEVICE_EDGE_CHILD" then
    local parent_device = device:get_parent_device()
    if device.preferences.changeConfigProfile == "Info" then
...
local function configuration_report_handler(driver, device, cmd)
  ...
  if device.network_type ~= "DEVICE_EDGE_CHILD" then
    ...
    --local parameter_value_set = cmd.args.configuration_value
    --local parameter_size_set = cmd.args.size
          ...
          for node = 1, max_nodes_supported do
            if node_ids[node] ~= nil then
              ...
              if node_ids[node] == child_device:get_field("node_value_set") and child_device:get_field("group_Command") == "RemoveOne" then
                ...
                child_device:emit_event(device_Info.deviceInfo({value = outputString}, {visibility = {displayed = true }}))
...
local function MultiChannelAssociation_report_handler(driver, device, cmd)
  ...
  if device.network_type ~= "DEVICE_EDGE_CHILD" then
    local child_device = device:get_child_by_parent_assigned_key("main")
    --print("<<device_Association_Type_value:",child_device:get_field("device_Association_Type_value"))
    if child_device ~= nil then
...
local function version_report_handler(driver, device, cmd)
  if device.network_type ~= "DEVICE_EDGE_CHILD" then
    --if device.zwave_manufacturer_id == 0x0109 and device.zwave_product_type == 0x2017 then 
...
--- peferences infoChanged
local function info_Changed(self, device, event, args)
  print("<<<< info_Changed in device-config subdriver >>>>")
  if device.network_type == "DEVICE_EDGE_CHILD" then
    local parent_device = device:get_parent_device()
    ...
    preferences.info_changed(self, device, args)
  end
end
